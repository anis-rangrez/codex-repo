...
- **Global scope**: `D:/Viblink/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Viblink/.claude/notes/`)
...
2. Glob `D:/Viblink/.claude/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `D:/Viblink/.claude/notes/` — works in any directory
