...
```bash
PHASE_INFO=$(node "D:/Viblink/.claude/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "D:/Viblink/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node "D:/Viblink/.claude/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
