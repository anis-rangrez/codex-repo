...
```bash
TRANSITION=$(node "D:/Viblink/.gemini/get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "D:/Viblink/.gemini/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
```bash
node "D:/Viblink/.gemini/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
