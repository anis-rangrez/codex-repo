...
```bash
AUDITOR_MODEL=$(node "D:/Viblink/.gemini/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-nyquist-auditor --raw)
NYQUIST_CFG=$(node "D:/Viblink/.gemini/get-shit-done/bin/gsd-tools.cjs" config get workflow.nyquist_validation --raw)
```
...
```
Task(
  prompt="Read D:/Viblink/.gemini/agents/gsd-nyquist-auditor.md for instructions.\n\n" +
...
**State B (create):**
1. Read template from `D:/Viblink/.gemini/get-shit-done/templates/VALIDATION.md`
...
node "D:/Viblink/.gemini/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```
