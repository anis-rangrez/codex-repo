...
```bash
INIT=$(node "D:/Viblink/.gemini/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "D:/Viblink/.gemini/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
